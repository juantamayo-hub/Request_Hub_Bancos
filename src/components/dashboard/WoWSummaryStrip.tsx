...
export function WoWSummaryStrip({ velocityLast7Days, avgResolutionDays, prevWeekAvgResolution }: Props) {
  ...
  const netStr  = net > 0 ? `neto +${net} resueltos` : net < 0 ? `neto ${net} pendientes` : 'neto 0 cambio'
  const avgStr  = avgResolutionDays > 0 ? `media ${avgResolutionDays}d resolución` : null
  const deltaStr =
  ...
  return (
    <p className="text-sm text-gray-400">
      <span className="font-medium text-gray-500">Esta semana — </span>
      {parts.join(' · ')}
    </p>
  )
}
